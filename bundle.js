var Sevenn = (() => {
  ...
  function createRichTextEditor({ value = "", onChange, ariaLabel, ariaLabelledBy } = {}) {
    ...
    function normalizeFontLabel(value2 = "") {
      if (!value2) return "Default";
      const primary = value2.split(",")[0] || "";
      return primary.replace(/^['"]+|['"]+$/g, "").trim() || "Default";
    }
    function parseFontSize(value2 = "") {
      ...
      runCommand(() => {
        try {
          if (typeof document.queryCommandSupported === "function" && document.queryCommandSupported("insertText")) {
            document.execCommand("insertText", false, normalized2);
            return;
          }
        } catch (err) {
        }
  ...
  function hasRichNodes(html = "") {
    ...
    if (typeof document === "undefined") {
      return /<(img|video|audio|iframe|canvas|svg|source)\b/i.test(html) || stripHtml2(html).length > 0;
    }
    const template = document.createElement("template");
    template.innerHTML = html;

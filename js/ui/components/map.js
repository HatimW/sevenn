...
function normalizeMapTab(tab = {}) {
  ...
  if (viewState) {
    normalized.lastView = { ...viewState };
  }
  normalized.lastViewAt = Number.isFinite(lastViewAt) && lastViewAt > 0 ? lastViewAt : 0;
  return normalized;
}
...
function applyInitialViewFocus(items = []) {
  ...
  if (newest) {
    ...
    if (pos) {
      ...
      viewBox.x = clamp(pos.x - width / 2, 0, maxX);
      viewBox.y = clamp(pos.y - height / 2, 0, maxY);
      viewBox.w = width;
      viewBox.h = height;
      constrainViewBox();
    }
...
function renderFilterControls(controls, activeTab) {
  ...
  if (!filteredLectures.length) {
    ...
    filteredLectures.forEach(lecture => {
      ...
      const weekLabel = Number.isFinite(lecture.week) ? `Week ${lecture.week}` : '';
      const option = createFilterOption({
